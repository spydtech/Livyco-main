...
const TenantRequestTable = () => {
        ...
        if (response.data.success && Array.isArray(response.data.bookings)) {
          ...
          const transformedBookings = response.data.bookings.map(booking => ({
            ...
            user: booking.user || {
              ...
              clientId: booking.user?.clientId || "N/A"
            },
            customerDetails: booking.customerDetails || {},
            propertyName: booking.property?.name || "N/A",
            roomType: booking.roomType || "N/A",
            durationType: booking.durationType || "monthly",
            bookingDetails: {
              roomNumber: booking.roomNumber || booking.roomDetails?.[0]?.roomNumber || "N/A",
              floor: booking.floor || booking.roomDetails?.[0]?.floor || "N/A"
            },
            moveInDate: booking.moveInDate,
            paymentStatus: booking.paymentStatus || booking.paymentInfo?.paymentStatus || "pending",
            paymentInfo: booking.paymentInfo || {},
            pricing: booking.pricing || {
              total: booking.pricing?.totalAmount || 0,
              advance: booking.pricing?.advanceAmount || 0,
              monthlyRent: booking.pricing?.monthlyRent || 0,
              securityDeposit: booking.pricing?.securityDeposit || 0
            },
            bookingStatus: booking.bookingStatus || "pending",
            transferStatus: booking.transferStatus || "not initiated",
            outstandingAmount: booking.outstandingAmount || 0,
            ...booking
          }));
          
          setTenants(transformedBookings);
        } else {
          setError(response.data.message || "Invalid data format received.");
        }
      } catch (err) {
  ...
  const filteredTenants = tenants.filter((tenant) => {
    ...
    const matchesBuilding = !filters.building || building === filters.building;
    const matchesRoomType = !filters.roomType || roomType === filters.roomType;
    const matchesFloor = !filters.floor || `${floor}` === filters.floor;
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {filteredTenants.length > 0 ? (
                  filteredTenants.map((tenant, index) => (
                    <tr key={tenant.id} className={`${index % 2 === 0 ? "bg-[#FFFFFF]" : "bg-[#F8F8FF]"}`}>
                      <td className="p-3">{tenant.user?.name || "N/A"}</td>
                      <td className="p-3">{tenant.user?.email || "N/A"}</td>
                      <td className="p-3">{tenant.moveInDate ? new Date(tenant.moveInDate).toLocaleDateString() : "N/A"}</td>
                      <td className="p-3">
                        <span className={`px-3 py-1 text-white text-sm rounded-md ${
                          tenant.paymentStatus === "paid" || tenant.paymentStatus === "confirmed" 
                            ? "bg-green-400" 
                      ...
                      <td className="p-3">₹{tenant.paymentInfo?.paidAmount?.toLocaleString() || "0.00"}</td>
                      <td className="p-3">{tenant.roomType || "N/A"}</td>
