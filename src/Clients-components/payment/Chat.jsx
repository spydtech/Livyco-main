...
export default function PaymentChat() {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        const transformedUsers = bookings.map(booking => ({
          _id: booking.user?.clientId || booking.id,
          ...
          outstandingAmount: booking.outstandingAmount || 0,
          lastPaymentDate: booking.payments?.find?.(p => p.status === 'completed')?.date,
          bookingId: booking.id, // Use booking.id!
        }));
  ...
  // Correct lookup for selected booking
        ...
        <div className="flex gap-16 h-screen">
            ...
            {bookingUsers.length === 0 ? (
              ...
              bookingUsers.map((booking) => (
                <div 
                  ...
                  className={`p-4 cursor-pointer mb-4 rounded-md shadow-md hover:bg-[#AFD1FF] ${booking.bookingId === selectedBookingId ? 'bg-[#AFD1FF] font-semibold' : ''}`}
                >
                  <div className="flex gap-2 items-center">
                    <img src={booking.profileImage} alt={booking.name} className="w-10 h-10 rounded-full object-cover" />
                    <div className="flex flex-col text-sm w-full">
                      <p className="font-bold mb-1">Payment From</p>
                      <div className="flex justify-between w-full text-gray-800">
                        <p>{booking.name}</p>
                        <p className="text-medium text-gray-500 mt-1">₹{booking.outstandingAmount || booking.rent}</p>
                      </div>
                    </div>
                  </div>
                  <div className="flex text-gray-500 items-center gap-2 text-xs mt-1">
                    <p>Room: {booking.roomNumber}</p>
                    <span className="border px-1 py-[1px] rounded text-[10px] font-bold">{booking.bed}</span>
                  </div>
                  {booking.lastPaymentDate && (
                    <p className="text-xs text-gray-500 mt-1">Last paid: {new Date(booking.lastPaymentDate).toLocaleDateString()}</p>
                  )}
                </div>
              ))
            )}
          ...
          {selectedBooking ? (
            <div className="flex-1 flex flex-col bg-white rounded-md shadow-md">
              <div className="border-b bg-[#AFD1FF] p-4 font-semibold flex items-center gap-4 justify-between">
                <div className='flex items-center gap-2'>
                  <img src={selectedBooking.profileImage} alt="profile" className="w-10 h-10 rounded-full object-cover" />
                  <div>
                    <p>{selectedBooking.name}</p>
                    <p className="text-xs text-gray-500">Room: {selectedBooking.roomNumber} {selectedBooking.bed}</p>
                    <p className="text-xs text-gray-500">Outstanding: ₹{selectedBooking.outstandingAmount || 0}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className='w-7 h-7 bg-[#0827B2] flex justify-center items-center rounded-full'><button className='cursor-pointer'><MessageCircle className="w-4 h-4 text-white" /></button></div>
                        ...
                        <div key={msg.id} className={`flex ${msg.sender === "You" ? "justify-end" : "justify-start"}`}>
                          <div className='flex flex-col'>
                            <div className="bg-[#AFD1FF] border border-blue-300 p-4 rounded-xl max-w-xs">
                              ...
                              <p className="text-2xl font-bold text-black">₹{msg.amount}/-</p>
                              <p className="text-xs text-gray-500 mt-1">
                                almost {getDaysLate(selectedBooking.lastPaymentDate)} day{getDaysLate(selectedBooking.lastPaymentDate) !== 1 ? 's' : ''} are over...
                              </p>
                            </div>
                            <div className='flex gap-2'>
                              <p className="text-[11px] mt-1">On {msg.date}</p>
              ...
              {requestpay && (
                <div className="flex-1 p-4">
                  <div className="flex flex-col items-center justify-center h-full space-y-4">
                    <img src={selectedBooking.profileImage} alt="profile" className="w-24 h-24 rounded-full object-cover mb-2" />
                    <p className="text-center font-semibold">Requesting Rent From {selectedBooking.name}</p>
                    <div className="border-2 px-4 rounded-lg text-center text-2xl font-semibold">₹{selectedBooking.outstandingAmount || selectedBooking.rent}</div>
                  </div>
                </div>
              )}
              {!requestpay && (
