...
import ClientNav from "../Client-Navbar/ClientNav";
import { vacateAPI } from "../PropertyController"; // Import the vacate API
...
const TenantCheckoutRequest = () => {
  ...
  const fetchVacateRequests = async () => {
    try {
      ...
      if (response.data && response.data.requests) {
        setVacateRequests(response.data.requests);
      } else {
        setVacateRequests([]);
      }
    } catch (err) {
      setError(err.message || "Failed to load vacate requests");
      console.error("Error fetching vacate requests:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredRequests = vacateRequests.filter((request) =>
      ...
      <>
        ...
        <div className="p-6 bg-[#F8F8FF] min-h-screen">
          <div className="flex justify-center items-center h-64">
            <div className="text-gray-600">Loading vacate requests...</div>
          </div>
        </div>
      </>
    );
  ...
  return (
    <>
      <ClientNav />
      <div className="p-6 bg-[#F8F8FF] min-h-screen">
        <h2 className="text-gray-600 text-sm mb-4">Home / Vacate Requests</h2>
        ...
        <div className="p-4 bg-white rounded-lg shadow">
          <div className="flex items-center mb-4">
            <input
              ...
              className="w-1/2 p-2 border rounded-3xl text-sm bg-white focus:outline-none focus:ring-1 focus:ring-gray-400"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
          </div>
          ...
          {filteredRequests.length === 0 ? (
            <div className="text-center py-10 text-gray-600">
              {vacateRequests.length === 0 ? "No vacate requests found" : "No matching requests found"}
            </div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="text-gray-600 text-sm border-b">
                  ...
                  <th className="p-3">Requested Check-out Date</th>
                  <th className="p-3">Outstanding Amount</th>
                  <th className="p-3">Expected Refund</th>
                  <th className="p-3">Action</th>
                </tr>
              </thead>
              <tbody>
                {filteredRequests.map((request) => (
                  <tr key={request.id} className="border-b hover:bg-gray-50">
                    <td className="p-3">{request.user?.name || 'N/A'}</td>
                    <td className="p-3">{request.user?.email || 'N/A'}</td>
                    <td className="p-3">{request.property?.name || 'N/A'}</td>
                    <td className="p-3">
                      {request.requestedDate ? new Date(request.requestedDate).toLocaleDateString() : 'N/A'}
                    </td>
                    <td className="p-3">₹{request.outstandingAmount || '0'}</td>
                    <td className="p-3">₹{request.securityDeposit || '0'}</td>
                    <td className="p-3">
                      <FiExternalLink 
                        className="text-gray-600 cursor-pointer hover:text-blue-600" 
                        onClick={() => navigate(`/client/approvecheckout/${request.id}`, { state: request })}
                      />
                    </td>
                  </tr>
                ))}
