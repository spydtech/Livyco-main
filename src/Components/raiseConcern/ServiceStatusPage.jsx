...
import { concernAPI } from "../../Clients-components/PropertyController";

const ServiceStatus = () => {
  const location = useLocation();
  const { concernId } = useParams();
  const [concernData, setConcernData] = useState(location.state);
  const [status, setStatus] = useState("raised");
  const [raisedAt] = useState(() => new Date());
  const [cancelledAt, setCancelledAt] = useState(null);
  ...
  useEffect(() => {
    const fetchConcernData = async () => {
      ...
      try {
        setLoading(true);
        const response = await concernAPI.getConcernById(concernId);
        
        if (response.data.success) {
          setConcernData(response.data.concern);
          setStatus(response.data.concern.status);
          
          if (response.data.concern.rejectedAt) {
            setCancelledAt(new Date(response.data.concern.rejectedAt));
          }
        }
      } catch (err) {
        setError(err.response?.data?.message || "Failed to fetch concern details");
  ...
  const handleCancel = async () => {
    ...
    try {
      const response = await concernAPI.cancelConcern(concernData._id || concernId);
      ...
      console.error("Error cancelling concern:", error);
  ...
  if (error || !concernData) {
    return (
      <>
        <Header />
        <div className="px-4 md:px-14 py-24 flex items-center justify-center">
          <p className="text-red-500">Error: {error || "No concern data found"}</p>
          <button 
  ...
  return (
    <>
          ...
          {type === "bed-change" && (
            <div className="flex items-center gap-10 justify-start mt-5 space-y-3">
              ...
              <div className="relative flex items-center justify-center w-14 h-10 rounded-lg overflow-hidden select-none transition">
                <div className="bg-[#FFBD15] absolute inset-0 rounded-lg" />
                <div className="absolute z-20 text-sm font-medium">
                  {requestedRoom}-{requestedBed?.replace("Bed ", "")}
                </div>
                <div
                  className="absolute right-0 top-0 w-3 h-6 bg-[#BCBCBC] rounded-sm"
                  style={{ transform: "translate(0.30rem, 33%)" }}
                  ...
                  {["Single", "Double", "Triple", "Four", "Five", "Six"].map(
                    (text) => (
                      <div
                        key={text}
                        className={`flex flex-col items-center justify-center h-20 border rounded-xl font-medium cursor-pointer transition
                          ${requestedSharingType === text.toLowerCase()
                            ? "bg-gray-300"
                            : "bg-white text-gray-500"
                          }`}
                      >
            ...
            <>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium">Final State</p>
                  <p className="text-xs text-gray-500">
                    {status === "cancelled" ? "Cancelled" : "Rejected"} at {formatDate(cancelledAt)}
                  </p>
                </div>
              </div>
            </>
      ...
      {(status === "raised" || status === "pending") && (
        <div className="flex justify-center items-center">
          <button
            className="bg-[#0827B2] text-white px-8 py-2 rounded-xl w-full sm:w-auto hover:bg-blue-700 disabled:bg-gray-400"
            onClick={handleCancel}
