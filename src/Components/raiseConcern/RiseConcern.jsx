...
// import { useNavigate } from "react-router-dom";
// import Header from "../Header";
// import { bookingAPI, propertyAPI } from "../../Clients-components/PropertyController";
...
//     // State for booking and property data
//     const [bookingData, setBookingData] = useState(null);
...
//                     // Debug: Check the booking structure
//                     console.log("Booking data structure:", latestBooking);
                    
//                     // Extract property ID from booking data - check different possible field names
//                     const propertyId = latestBooking.propertyId || 
//                                       latestBooking.property?._id || 
//                                       latestBooking.propertyDetails?._id;
                    
//                     if (!propertyId) {
//                         console.error("Property ID not found in booking data:", latestBooking);
//                         setError("Property information not available in your booking data");
//                         return;
//                     }
...
//                             // The properties are in response.data.data
//                             const properties = propertyResponse.data.data || [];
                            
//                             // Find the specific property that matches the booking's propertyId
//                             const property = properties.find(
//                                 prop => prop.property && prop.property._id === propertyId
//                             );
...
//                                 await fetchAvailableBeds(propertyId);
//                             } else {
//                                 console.error("Property not found in response. Booking propertyId:", propertyId);
//                                 console.error("Available properties:", properties.map(p => p.property._id));
//                                 setError("Property data not found for your booking. Please contact support.");
//                             }
//                         } else {
//                             setError("Failed to fetch property data: " + (propertyResponse.data.message || "Unknown error"));
//                         }
//                     } catch (propErr) {
//                         console.error("Error fetching property data:", propErr);
//                         setError("Failed to fetch property data: " + propErr.message);
//                     }
//                 } else {
...
//     // Function to fetch available beds
//     const fetchAvailableBeds = async (propertyId) => {
//         try {
...
//                 setError("Please log in to check bed availability");
//                 return;
//             }
            
//             // Use the endpoint that returns all beds
//             const response = await fetch(`http://localhost:5000/api/auth/bookings/availability/property/${propertyId}/all-beds`, {
//                 method: 'GET',
//                 headers: {
...
//     // Generate beds based on sharing type for bed change - using actual bed numbers
//     const beds = useMemo(() => {
//         if (!currentSharing || !propertyData || !propertyData.rooms) return [];
        
//         // Get the actual bed numbers from the property data for the current room
//         const roomConfig = propertyData.rooms.floorConfig?.floors?.[0]?.rooms || {};
//         const currentRoomBeds = roomConfig[currentRoom] || [];
        
//         return currentRoomBeds.map((bedName, index) => {
//             const bedNumber = bedName.replace("Bed ", ""); // Extract just the number
//             const bedId = `${currentRoom}-${bedNumber}`;
//             const bedIdentifier = `${currentSharing}-${currentRoom}-${bedName}`;
//             const isBooked = bedStatus[bedIdentifier] === 'booked' || 
//                             bedStatus[bedIdentifier] === 'approved' || 
//                             bedNumber === currentBed;
            
//             return {
//                 id: bedId,
//                 room: currentRoom,
//                 bed: bedNumber, // Use the actual bed number (1, 2, 3, etc.)
//                 booked: isBooked,
//                 identifier: bedIdentifier
//             };
//         });
//     }, [currentSharing, currentRoom, currentBed, propertyData, bedStatus]);

//     // Generate availability data from property data for room change
//     const sharingFloors = useMemo(() => {
//         if (!propertyData || !propertyData.rooms || !propertyData.rooms.floorConfig) return {};
        
//         const floorsData = {};
        
//         // Process each room type
//         propertyData.rooms.roomTypes.forEach(roomType => {
//             const sharingType = roomType.type;
//             const sharingLabel = roomType.label.replace(" Sharing", "");
            
//             // Get floors for this room type
//             const floors = propertyData.rooms.floorConfig.floors.map(floor => {
//                 const beds = [];
                
//                 // Process each room on this floor
//                 Object.entries(floor.rooms).forEach(([roomNumber, roomBeds]) => {
//                     // Check if this room is of the current type
//                     const roomTypeMatch = propertyData.rooms.floorConfig.selectedRooms.includes(sharingType);
                    
//                     if (roomTypeMatch) {
//                         // Add each bed in this room
//                         roomBeds.forEach((bedName, index) => {
//                             const bedNumber = bedName.replace("Bed ", ""); // Extract just the number
//                             const bedId = `${roomNumber}-${bedNumber}`;
//                             const bedIdentifier = `${sharingType}-${roomNumber}-${bedName}`;
//                             const isBooked = bedStatus[bedIdentifier] === 'booked' || bedStatus[bedIdentifier] === 'approved';
...
//             floorsData[sharingLabel] = floors;
//         });
        
//         return floorsData;
//     }, [propertyData, bedStatus]);

//     // For bed change, only show beds from the current room
//     const visibleBeds = useMemo(() => {
//         return beds.filter(bed => bed.room === currentRoom);
//     }, [beds, currentRoom]);

//     // Get room types from property data or use fallback
//     const roomTypes = useMemo(() => {
//         return propertyData?.rooms?.roomTypes || [
//             { type: "single", label: "Single Sharing", capacity: 1, price: 5000, deposit: 2000 },
//             { type: "double", label: "Double Sharing", capacity: 2, price: 4000, deposit: 2000 },
//             { type: "triple", label: "Triple Sharing", capacity: 3, price: 3000, deposit: 2000 },
//             { type: "four", label: "Four Sharing", capacity: 4, price: 2500, deposit: 2000 }
//         ];
//     }, [propertyData]);

//     const toggleBed = (id) => {
//         const selectedBed = visibleBeds.find(b => b.id === id);
//         if (selectedBed && selectedBed.booked) {
//             alert("This bed is not available. Please select another bed.");
//             return;
//         }
//         setSelectedId((prev) => (prev === id ? null : id));
//     };
...
//     const toggleRoomChangeBed = (id) => {
//         const selectedBed = roomChangeFloorBeds.find(b => b.id === id);
//         if (selectedBed && selectedBed.booked) {
//             alert("This bed is not available. Please select another bed.");
//             return;
//         }
...
//             setRequestData({
//                 type: service,
//                 bed: `Bed ${selected.bed}`, // Return the full bed name with "Bed" prefix
//                 room: selected.room,
//                 originalRoom: currentRoom,
//                 originalBed: `Bed ${currentBed}`,
//             });
//             setShowPopup(true);
//         } else if (service === "room-change") {
//             if (!selectedSharing) {
...
//                                                     key={roomType.type}
//                                                     onClick={() => {
//                                                         setSelectedSharing(sharingLabel);
//                                                         setSelectedRoomChangeBedId(null);
//                                                         const floors = sharingFloors[sharingLabel] || [];
//                                                         setSelectedFloor(floors[0]?.floor || null);
//                                                     }}
//                                                     className={`flex flex-col items-center justify-center h-20 border rounded-xl font-medium cursor-pointer transition
//                                                         ${selectedSharing === sharingLabel
//                                                             ? "bg-gray-300 text-black"
//                                                             : "bg-white text-gray-500"
//                                                         }`}
//                                                 >
...
//                                 {/* Show message if no property data available */}
//                                 {!propertyData && (
//                                     <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4">
//                                         <p>Property data not available. Please contact support for room change options.</p>
//                                     </div>
//                                 )}

//                                 {/* Floor Tabs - Only show if property data is available */}
//                                 {propertyData && selectedSharing && (
//                                     <div>
//                                         <h4 className="font-medium mb-2">Select Floor</h4>
//                                         <div className="flex gap-3 flex-wrap">
//                                             {(sharingFloors[selectedSharing] || []).map((f) => (
...
//                                 {/* Bed selection for room-change - Only show if property data is available */}
//                                 {propertyData && selectedSharing && selectedFloor && (
//                                     <div className="p-4 bg-white rounded-md">
//                                         <h3 className="text-sm mb-5">
//                                             Select preferred bed for the new sharing on Floor{" "}
//                                             {selectedFloor}
...
const RiseConcern = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (response.data.success && response.data.bookings.length > 0) {
                    ...
                    // Extract property ID from booking data
                    const propertyId = latestBooking.propertyId || 
                                      latestBooking.property?._id || 
                                      latestBooking.propertyDetails?._id;
    ...
    // Generate beds based on sharing type for bed change
    const beds = useMemo(() => {
        if (!currentSharing || !propertyData || !propertyData.rooms) return [];
        ...
        return currentRoomBeds.map((bedName, index) => {
            const bedNumber = bedName.replace("Bed ", "");
            const bedId = `${currentRoom}-${bedNumber}`;
            const bedIdentifier = `${currentSharing}-${currentRoom}-${bedName}`;
            const isBooked = bedStatus[bedIdentifier] === 'booked' || 
                            bedStatus[bedIdentifier] === 'approved' || 
    ...
    const handleRaiseRequest = async () => {
        ...
        try {
            ...
            if (service === "bed-change") {
                const selected = visibleBeds.find((b) => b.id === selectedId);
                if (!selected || selected.booked) {
                    alert("Please select a valid available bed.");
                    return;
                }
                
                concernData = {
                    ...concernData,
                    requestedRoom: selected.room,
                    requestedBed: `Bed ${selected.bed}`
                };
                ...
                const selected = floorInfo.beds.find((b) => b.id === selectedRoomChangeBedId);
                if (!selected || selected.booked) {
                    alert("Please select a valid available bed.");
                    return;
                }
                ...
                concernData = {
                    ...
                    requestedRoom: selected.room,
                    requestedBed: `Bed ${selected.bed}`,
                    requestedSharingType: selectedSharing.toLowerCase(),
            ...
            // Submit the concern to the backend
            const response = await concernAPI.submitConcern(concernData);
            
            if (response.data.success) {
                setRequestData({
                    ...concernData,
                    concernId: response.data.concern._id
                });
                setShowPopup(true);
            } else {
                alert("Failed to submit concern: " + response.data.message);
            }
            
        } catch (error) {
            console.error("Error submitting concern:", error);
            alert("Failed to submit concern: " + (error.response?.data?.message || error.message));
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleConfirmPopup = () => {
        ...
        // Navigate to the concern status page with the concern data
        navigate(`/user/concern/${requestData.concernId}`, {
            state: {
                ...requestData,
                currentRoom,
                currentBed: `Bed ${currentBed}`,
                currentSharing,
                concernId: requestData.concernId
            },
        });
    };
        ...
        <div className="px-4 md:px-14 py-24 flex flex-col justify-between relative">
            ...
            <div className="flex justify-center items-center mt-16">
                <button
                    className={`px-8 py-2 rounded-xl w-full sm:w-auto transition 
                        ${isValid && !submitting
                            ? "bg-[#0827B2] text-white cursor-pointer hover:bg-blue-700"
                            : "bg-gray-400 text-gray-700 cursor-not-allowed"
                        }`}
                    onClick={handleRaiseRequest}
            ...
            {showPopup && (
                <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4">
                    <div className="bg-white p-6 rounded-lg text-center shadow-xl space-y-10 w-full max-w-xl">
                        ...
                        />
                        <button
                            className="bg-[#0827B2] text-white w-full py-2 rounded-lg hover:bg-blue-700"
                            onClick={handleConfirmPopup}
                        >
                            Okay, Got it!
                        </button>
