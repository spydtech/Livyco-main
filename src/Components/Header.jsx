...
const Header = () => {
  ...
  // Fetch user data - FIXED: Don't redirect on home page
  useEffect(() => {
    const fetchUserData = async () => {
      const token = localStorage.getItem('token');
      
      // No token = not logged in, just return
      if (!token) {
        setUser(null);
        setLoading(false);
        return;
      ...
      try {
        ...
        if (response.data.success) {
          ...
          // If API fails, clear token but don't redirect
          localStorage.removeItem('token');
          setUser(null);
        }
      } catch (error) {
        ...
        // Only clear token, NO REDIRECT on home page
        if (error.response?.status === 401) {
          localStorage.removeItem('token');
        }
        setUser(null);
    ...
    <>
      <header className="w-full backdrop-blur-lg bg-[#0019A5]/90 text-white shadow-md fixed z-50">
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-0 py-0 flex items-center justify-between">
          ...
          <ul className="hidden md:flex gap-8 items-center text-sm font-medium">
            ...
            {isLoggedIn ? (
              <>
                ...
                <li 
                  className="flex items-center gap-1 hover:text-yellow-400 hover:border-b-2 border-yellow-400 pb-1 transition duration-200 cursor-pointer"
                  onClick={() => handleProtectedNav('/user/chats')}
                >
                  <IoChatbubbleOutline className='text-xl'/> Chats
                </li>
                ...
                <li 
                  className="flex items-center gap-1 hover:text-yellow-400 hover:border-b-2 border-yellow-400 pb-1 transition duration-200 cursor-pointer"
                  onClick={() => handleProtectedNav('/user/pgsearch')}
                >
                  <HiOutlineHome className='text-xl'/> Hostel
                </li>
                ...
                <li 
                  className="flex items-center gap-1 hover:text-yellow-400 hover:border-b-2 border-yellow-400 pb-1 transition duration-200 cursor-pointer"
                  onClick={() => handleProtectedNav('/user/wishlist')}
                >
                  <FaRegHeart className='text-xl'/> Wishlist
                </li>
                ...
                <li className="relative">
                  ...
                  >
                    ...
                    {isLoggedIn && unreadCount > 0 && (
                      <span className="absolute -top-1 -right-1 min-w-4 h-4 bg-red-500 rounded-full text-xs flex items-center justify-center text-white px-1 font-medium border border-white">
                ...
                <li className="relative">
                  ...
                  {profileDropdownOpen && (
                    ...
                    >
                      <div className="px-4 py-2 border-b border-yellow-300">
                        <p className="text-sm font-medium text-white">Hello, {user?.name || "User"}</p>
                        <p className="text-xs text-white truncate">{user?.email || ""}</p>
                      </div>
                      <div 
                        className="flex items-center gap-2 px-4 py-2 text-sm text-white hover:text-blue-800 cursor-pointer transition-colors duration-200 hover:bg-yellow-300"
                        onClick={() => handleProtectedNav('/user/profile')}
                      >
                        <FaRegUser className='text-lg' />
                        <span>Profile</span>
                      </div>
              ...
              <>
                {/* Show only Get App and Login for non-logged in users */}
                <li className="flex items-center gap-1 hover:text-yellow-400 hover:border-b-2 border-yellow-400 pb-1 transition duration-200">
                  <BsPhone className='text-xl'/> Get App
                </li>
                <li>
          ...
          {/* Mobile Navigation Icons */}
          <div className="flex md:hidden items-center gap-3">
            {/* Notification Icon (only show if logged in) */}
            {isLoggedIn && (
              <button 
                onClick={toggleNotifications}
                className="relative p-2 rounded-full hover:bg-white/10 transition duration-200"
        ...
        {menuOpen && (
          <div className="md:hidden bg-[#0019A5] backdrop-blur-lg border-t border-white/20">
            <ul className="flex flex-col gap-2 p-4 text-sm font-medium">
                ...
                <>
                  ...
                  >
                    <div className="relative">
                      <FaBell className="text-lg" />
                      {isLoggedIn && unreadCount > 0 && (
                        <span className="absolute -top-2 -right-2 min-w-4 h-4 bg-red-500 rounded-full text-xs flex items-center justify-center text-white px-1 font-medium border border-white">
                          {formatNotificationCount(unreadCount)}
                        </span>
                      )}
                    </div>
              ...
              {/* Get App is visible to all users */}
              <li className="flex items-center gap-3 p-3 rounded-lg hover:bg-white/10 transition duration-200">
                <BsPhone className="text-lg" /> Get App
              </li>
      ...
      {/* Notification Panel - Only shows when logged in */}
      {isLoggedIn && (
        <>
          <div 
            ref={notificationsRef}
