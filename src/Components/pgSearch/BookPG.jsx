...
import { Plane, TrainFront, Bus, Hospital, ShoppingCart } from "lucide-react";
import bgImg from '../../assets/user/pgsearch/image (5).png';
import reviewImg from "../../assets/reviews-icons/undraw_reviews_ukai.png";
import mapImg from "../../assets/map/undraw_map_cuix.png";
import { useNavigate } from "react-router-dom";
import { useParams } from "react-router-dom";
import { useLocation } from "react-router-dom";
import Header from "../Header";
...
export default function BookPG() {
  ...
  // Fetch reviews for this property - PROPERLY INTEGRATED
  const fetchPropertyReviews = async () => {
    const propertyId = getPropertyId();
    
    if (!propertyId || propertyId === 'undefined') {
      console.log('No valid property ID for reviews:', { pg, id });
      setReviewsLoading(false);
      setReviewsError("Property ID not available");
      return;
    }
    ...
    try {
      ...
      // Use the reviewAPI instead of direct fetch calls
      const response = await reviewAPI.getPropertyReviews(propertyId, 'approved');
      ...
      if (reviewsData.length > 0) {
        ...
        setReviews([]);
        setReviewsError("No reviews found for this property");
      }
    } catch (error) {
      ...
      if (error.response?.status === 404) {
        ...
        setReviewsError("Server error. Please try again later.");
        setReviews([]);
      } else {
        setReviewsError(error.message || "Failed to load reviews. Please try again.");
        setReviews([]);
      }
    } finally {
      setReviewsLoading(false);
  ...
  const handleSubmitReview = async (reviewData) => {
    if (!user) {
      navigate("/user/login", {
        state: { from: `/pg/${id}`, message: "Please login to submit a review" }
      });
      return;
    }
    ...
    try {
      const reviewPayload = {
        ...
        userAvatar: user.profilePicture || user.avatar
      };
  ...
  // Get reviews to display (first 3 when collapsed, all when expanded)
        ...
        <div className="flex flex-col mx-10 md:mx-44">
            ...
            <div className="w-full md:w-3/5 flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                ...
                <div className="text-yellow-500 py-1">
                  ...
                  <span className="text-gray-600 text-sm ml-2">({reviewStats.totalReviews} reviews)</span>
                </div>
                <p className="text-sm text-gray-600">
                  {pg.pgProperty?.description || pg.description || "Comfortable and affordable PG accommodation with all modern amenities."}
                </p>
          ...
          <div className="flex justify-between items-center bg-white py-4 mt-5 rounded-lg shadow-sm px-6">
            ...
            {user && (
              <button
                onClick={() => {
                  // You can implement a review modal here
          ...
          {/* Increased gap between header and reviews */}
          ...
          {reviewsLoading ? (
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm text-center">
              <div className="text-yellow-500 text-4xl mb-4">
                 <img src={reviewImg} alt="No Reviews" className="w-[25%] h-auto mx-auto" />
              </div>
            ...
            <div className="bg-white p-8 rounded-lg shadow-sm text-center">
              <div className="text-gray-400 text-4xl mb-4">
                <img src={reviewImg} alt="No Reviews" className="w-26 h-auto mx-auto" />
              </div>
            ...
            <>
              {/* Reviews Grid */}
              <div className="grid grid-cols-1 bg-white md:grid-cols-3 p-6 gap-16 rounded-lg shadow-sm">
                {getDisplayedReviews().map((review) => (
                  <div key={review._id} className="relative min-h-44 overflow-visible">
                    <div className="bg-gray-700 w-full rounded-tr-xl rounded-bl-xl absolute z-10 pb-5 pt-12">
                      <div className="flex flex-col justify-center items-center text-white gap-1 px-4">
                        <p className="font-semibold mt-3">{review.userName || "Anonymous User"}</p>
                        {renderStars(review.rating || 0)}
                        <p className="text-sm text-center mt-1">"{review.comment || "No comment provided"}"</p>
                        {review.adminResponse && (
                          <div className="mt-2 p-2 bg-blue-900 rounded text-xs w-full">
                            <strong>Owner: </strong>{review.adminResponse}
                          </div>
                        )}
              ...
              {reviews.length > 3 && (
                <div className="flex justify-center mt-12 mb-6">
                  <button
                    ...
                    className="bg-[#facc14] text-black px-6 py-2 rounded-lg hover:bg-[#facc14] transition-colors font-medium"
                  >
                ...
                {mapLoading ? (
                  ...
                  <div className="p-6 text-center">
                    <div className="  p-4 max-w-md mx-auto">
                      <div>
                        <img src={mapImg} alt="Error" className="w-26 h-auto mx-auto" />
                         </div>
                  ...
                  <div className="p-8 text-center">
                    <div className="text-4xl mb-4">
                      <image src={mapImg} alt="No Location" className="w-12 h-auto mx-auto" />
                    </div>
                    <h3 className="text-xl font-semibold mb-2">Location Not Available</h3>
                    <p className="text-gray-600">
                      Location details for this property are not available.
